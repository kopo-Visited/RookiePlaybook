...
function DashboardPage() {
  ...
  function handleShortcut(s) {
    if (s.action === 'qna') { setQnaModalOpen(true); return; }
    ...
    <div className={styles.page}>
      ...
      <div className={styles.welcomeBanner}>
        <div className={styles.welcomeLeft}>
          ...
          <p className={styles.welcomeSub}>오늘도 신입의 정석과 함께 성장해 보세요. 진행 중인 교육과 문서를 확인하세요.</p>
        </div>
      ...
      <div className={styles.midRow}>
        ...
        <section className={styles.card}>
          <div className={styles.sectionHead}>
            ...
            <button className={styles.linkBtn} onClick={() => navigate(ROUTES.EDU.LIST)}>전체보기 ›</button>
          </div>
          <ul className={styles.eduList}>
            ...
            {inProgressEdu.map((edu, i) => (
              <li key={edu.educationId ?? i} className={styles.eduItem}>
                <div className={styles.eduIcon} style={{ background: EDU_COLORS[i % EDU_COLORS.length] }}>
                  <span style={{ fontSize: 24, color: EDU_ICON_COLORS[i % EDU_ICON_COLORS.length] }}>📖</span>
                </div>
                <div className={styles.eduBody}>
                  <div className={styles.eduTitleRow}>
                    <span className={styles.eduTitle}>{edu.title}</span>
                    <span className={styles.badge} style={{ background: '#DFF1FF', color: '#2288FF' }}>진행중</span>
                  </div>
        ...
        <section className={styles.card}>
          <div className={styles.sectionHead}>
            ...
            <button className={styles.linkBtn} onClick={() => navigate(ROUTES.DOC.LIST)}>전체보기 ›</button>
          </div>
          <ul className={styles.docList}>
            ...
            {recentDocs.map((doc, i) => (
              <li key={doc.id ?? i} className={styles.docItem}>
                <span className={styles.docTypeBadge} style={{ background: '#EAF4FF', color: '#2288FF' }}>DOC</span>
        ...
        <section className={styles.card}>
          <div className={styles.sectionHead}>
            ...
            <button className={styles.linkBtn} onClick={() => navigate(ROUTES.QNA.LIST)}>전체보기 ›</button>
          </div>
                ...
                <li key={q.questionId ?? i} className={styles.qnaItem}>
                  ...
                  <div className={styles.qnaBody}>
                    ...
                    <span className={styles.qnaMeta}>
                      <span className={styles.qnaStatus} style={{ color: st.color }}>{st.label}</span>
          ...
          <button className={styles.btnOutline} onClick={() => setQnaModalOpen(true)}>+ 질문하기</button>
        </section>
      </div>
          ...
          <ul className={styles.noticeList}>
            {[
              ...
              <li key={i} className={styles.noticeItem}>
                <span className={styles.noticeTitle}>
                  <span style={{ color: '#2288FF', fontWeight: 900, marginRight: 6 }}>Q.</span>{q}
                </span>
              </li>
            ))}
          </ul>
